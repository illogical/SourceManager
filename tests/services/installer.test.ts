...
function makeService(overrides: { repoPath?: string; packageManager?: string; scriptName?: string; installCommand?: string } = {}) {
  return {
    repoPath: overrides.repoPath ?? tmpDir,
    packageManager: overrides.packageManager ?? "auto",
    scriptName: overrides.scriptName ?? "dev",
    installCommand: overrides.installCommand,
  } as { repoPath: string; packageManager: string; scriptName: string; installCommand?: string }
}
...
describe("runInstall", () => {
  ...
  it("returns success on exit code 0", async () => {
    ...
    const dir = makeProjectDir(["bun.lockb"])
    const svc = makeService({ repoPath: dir, packageManager: "bun" })
    const result = await runInstall(svc.repoPath, svc)
  ...
  it("returns failure when install exits with non-zero code", async () => {
    ...
    const svc = makeService({ packageManager: "npm" })
    const result = await runInstall(svc.repoPath, svc)
  ...
  it("uses custom installCommand (split on spaces)", async () => {
    mockSpawn(0)
    const svc = makeService({ installCommand: "npm ci" })
  ...
  it("reports success message containing the command used", async () => {
    mockSpawn(0)
    const svc = makeService({ installCommand: "pnpm install --frozen-lockfile" })
    const result = await runInstall(svc.repoPath, svc)
    expect(result.status).toBe("success")
